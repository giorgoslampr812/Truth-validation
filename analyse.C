...
void analyse() {
    ...
    TFile *f = TFile::Open("Run2_ZZ_fid_100k.root");
    ...
    // Fat jets
    std::vector<double> *jet_dick_pt=0,*jet_dick_eta=0,*jet_dick_phi=0,*jet_dick_m=0;

    t->SetBranchAddress("jet_dick_pt",&jet_dick_pt);
    t->SetBranchAddress("jet_dick_eta",&jet_dick_eta);
    t->SetBranchAddress("jet_dick_phi",&jet_dick_phi);
    t->SetBranchAddress("jet_dick_m",&jet_dick_m);
	...
	Gen_sum[k] = Gen_sum[k] + genWeights->at(k);
        ...
        for(size_t l=0;l<lep_pt->size();l++){
            ...
            if(lep_pt->at(l)>27 && fabs(lep_eta->at(l))<2.5)
                good_leptons++; 	         
        }
        double deta = lep_eta->at(1) - lep_eta->at(0);
        double dphi = lep_phi->at(1) - lep_phi->at(0);
        double DR_le = sqrt(deta*deta + dphi*dphi);
        //if(good_leptons<2) continue; //pid and DR_le cuts have no effect 
	if(good_leptons<2 || DR_le < 0.2 || lep_pid->at(0)==lep_pid->at(1)) continue;
        ...
        for(size_t j=0;j<jet_pt->size();j++){
            ...
            double eta=fabs(jet_eta->at(j));
            
	    double eta2 = jet_eta->at(j);
	    double phi2 = jet_phi->at(j);
	    double deta1 = eta2 - lep_eta->at(0);
	    double dphi1 = phi2 - lep_phi->at(0);
	    double DR1 = sqrt(deta1*deta1 + dphi1*dphi1);
	    double deta2 = eta2 - lep_eta->at(1);
            double dphi2 = phi2 - lep_phi->at(1);
	    double DR2 = sqrt(deta2*deta2 + dphi2*dphi2);
	    
            if(eta<2.5 && pt>20 && DR1>0.2 && DR2>0.2) good_jets.push_back(j); //Jet overlap may help
            if(eta>2.5 && eta<4.5 && pt>30 && DR1>0.2 && DR2>0.2) good_jets.push_back(j); // Jet overlap may help
	...
	double fat_phi = 0;
        ...
        for(size_t j=0;j<jet_dick_pt->size();j++){

            double pt=jet_dick_pt->at(j);
            double eta=fabs(jet_dick_eta->at(j));
	    double ma = jet_dick_m->at(j);
	    
            double eta2 = jet_dick_eta->at(j);
            double phi2 = jet_dick_phi->at(j);
	    ...
	    double DR_av = (DR1+ DR2)/2;
            if((pt>200) && (eta<2.0) && (DR3>1.0) && (ma > 50)){ 
            //if((pt>200) && (eta<2.0) && (ma > 50)){
                if(pt>fat_pt){

                    fat_pt=pt;
                    fat_eta=jet_dick_eta->at(j);
                    fat_m=jet_dick_m->at(j);
		    fat_phi = jet_dick_phi->at(j);
                }
            }
        }
	...
	double deta3 = fat_eta - lep_eta->at(0);
        ...
        double DR3 = sqrt(deta3*deta3 + dphi3*dphi3);
	//if ((fat_m > 64) && (fat_m < 106) && (DR1 >1.4) && (DR2 >1.4)){ //Doable only with DR_av
	if (fat_m > 64 && fat_m < 106 ){
