...
namespace Rivet {
  ...
  class VBS_fid : public Analysis {
  public:

    /// Constructor
    RIVET_DEFAULT_ANALYSIS_CTOR(VBS_fid);
  ...
  return (sumPt > 0) ? sumPtDR / sumPt : 0.0;
    ...
    InvisibleFinalState neutrinos; // check whether second true is needed
    declare(neutrinos, "neutrinos");
       ...
       declare(jets, "Jets");
       FastJets jets_dick(vfs, FastJets::ANTIKT, 1.0);
       declare(jets_dick, "Jets_dick");
...
//--------tagging jet variables--------------------------------------------
      ...
      _file = new TFile("/data/glamprinoudis/RIVET/Run2_WZ_1lep_fid_100k.root", "RECREATE");
      _tree = new TTree("events", "Rivet ntuple");
      // --Your mom is a fat jet 
      _tree->Branch("jet_dick_pt", &_jet_dick_pt);
      _tree->Branch("jet_dick_m", &_jet_dick_m);
      _tree->Branch("jet_dick_eta", &_jet_dick_eta);
      _tree->Branch("jet_dick_phi", &_jet_dick_phi);
      _tree->Branch("jet_dick_E", &_jet_dick_E);
      _tree->Branch("jet_dick_y", &_jet_dick_y);
      _tree->Branch("Jet_dick_m", &_Jet_dick_m);
      _tree->Branch("Jet_dick_pt", &_Jet_dick_pt);
      _tree->Branch("Jet_dick_eta", &_Jet_dick_eta);
      _tree->Branch("Jet_dick_phi", &_Jet_dick_phi);
    ...
    _h_cutflow->Fill(1.0,1.0);
       ...
       Jets jets_dick = apply<FastJets>(event, "Jets_dick").jetsByPt(Cuts::pT > 0.0*GeV && Cuts::abseta < 4.5);
      ...
      sort(jets_dick.begin(), jets_dick.end(), [](const Jet& a, const Jet& b) {
         return a.pT() > b.pT();
      });
    ...
    for (const Jet& j2 : jets_dick) {
     _jet_dick_pt.push_back(j2.pt());
     _jet_dick_m.push_back(j2.mass());
     _jet_dick_phi.push_back(j2.phi());
     _jet_dick_eta.push_back(j2.eta());
     _jet_dick_E.push_back(j2.E());
     _jet_dick_y.push_back(j2.rap());
    } 
   
    //Vector filling
    _Jet_dick_m = jets_dick[0].mass();
    _Jet_dick_pt = jets_dick[0].pt();
    _Jet_dick_eta = jets_dick[0].eta();
    _Jet_dick_phi = jets_dick[0].phi();
    _Jet_dick_E = jets_dick[0].E();
    _Jet_dick_y = jets_dick[0].rap();
    _Jet_n = jets.size();
    _Miss_ET = etmiss;
    _Btags = btags;
    _Jet1_m = jets[0].mass();
...
// Scale histograms using femtobarn unit
    ...
    std::vector<double> _jet_dick_pt, _jet_dick_m, _jet_dick_eta, _jet_dick_phi, _jet_dick_E, _jet_dick_y;
    std::vector<double> _jet_pt, _jet_m, _jet_eta, _jet_phi, _jet_E, _jet_y;
    unsigned long long _Jet_n;
    double _Jet_dick_m, _Jet_dick_pt, _Jet_dick_eta, _Jet_dick_phi, _Jet_dick_E, _Jet_dick_y;
